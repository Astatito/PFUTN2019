...
import { View, StyleSheet, TextInput, StatusBar, FlatList, Alert } from 'react-native';
import { Database } from '../../Firebase';
import { LocalStorage } from '../../Storage';
import Icon from 'react-native-vector-icons/MaterialIcons';
import { Content, Button, Text, Picker, ListItem, Left, Body, Right, Thumbnail} from 'native-base';
...
const flatListData = [
    ...
    },
    {
        ...
        fechaDesde: '02/11/1993',
        fechaHasta: '01/34/3411'
    },
    {
        nombre: 'Club House ',
        estado: 'No Disponible',
        fechaDesde: '02/11/1993',
        fechaHasta: '01/32/3114'
    }
]
                ...
                <Swipeout {...swipeOutSettings}>
                    <ListItem avatar>
                        ...
                        <Body style={{ alignSelf: 'center' }}>
                            ...
                            <Text style={{ fontSize: 14 }}> {this.props.item.estado} </Text>
                        </Body>
...
export default class BasicFlatList extends Component {
    ...
    state = { 
        ...
        picker: '',
        tiposServicio: [],
        fechaReserva: moment(new Date()).format('MMM Do YY'),
        documento: '',
        showSpinner: false,
        isFocused: false,
        isVisible: false,
    ...
    // Acá debería obtener los distintos servicios.
    obtenerPickers = () => {
        var dbRef = Database.collection('TipoDocumento');
        var dbDocs = dbRef.get().then(snapshot => {
            var tiposServicio = [];
            snapshot.forEach(doc => {
                tiposServicio.push({ id: doc.id, nombre: doc.data().Nombre });
            });
            this.setState({ tiposServicio });
        });
    };
    ...
    handlePicker = (datetime) => {
        this.setState({
            isVisible: false,
            fechaReserva : moment(datetime).format('MMM Do YY')
        })
    }
    ...
    render() {
        ...
        if (this.state.tiposServicio.length < 3) {
            this.obtenerPickers();
        }
        ...
        return (
            <View>
                ...
                <View style={styles.container}>
                    ...
                    <Text style={styles.header}> Registrar nueva reserva </Text>
                    <Picker
                        note
                        mode="dropdown"
                        style={styles.picker}
                        ...
                        <Picker.Item label="Servicio" value="-1" color="#7B7C7E" />
                        {this.state.tiposServicio.map((item, index) => {
                            return <Picker.Item label={item.nombre} value={item.id} key={index} />;
                        })}
                    </Picker>

                    <View style={styles.datetime}>
                        <Text style={{alignSelf:'center', color: '#8F8787'}}>Fecha de reserva</Text>
                        <Text style={{alignSelf:'center', color:'#1e90ff', paddingHorizontal: '7    %', fontSize:15}}> {this.state.fechaReserva} </Text>
