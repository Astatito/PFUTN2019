import React, { Component } from 'react';
import { View, Alert, FlatList, FlatListItem} from 'react-native';
import { Container, Button, Content, List, ListItem, Left, Body, Right, Thumbnail, Text, Separator, Icon} from 'native-base';
import Swipeout from 'react-native-swipeout';
import { ScrollView } from 'react-native-gesture-handler';

class Invitaciones extends Component {

    static navigationOptions = {
        title: 'Invitaciones',
    };

    constructor(props) {
        this.state= {
            activeRowKey : null , invitados : [
                {documento: '40.894.499' , fechaDesde: '02/11/2018 20:00 hs', fechaHasta: '02/11/2018 23:00 hs'},
                {documento: '38.599.552' , fechaDesde: '02/11/2018 20:00 hs', fechaHasta: '02/11/2018 23:00 hs'},
                {documento: '39.611.837' , fechaDesde: '02/11/2018 20:00 hs', fechaHasta: '02/11/2018 23:00 hs'},
            ], invitadosAutenticados: [
                {nombre: 'Alexis' , apellido: 'Pagura', documento: '39.645.758' , fechaDesde: '02/11/2018 20:00 hs', fechaHasta: '02/11/2018 23:00 hs'},
                {nombre: 'Fabián' , apellido: 'Guidobaldi',documento: '40.875.659' , fechaDesde: '02/11/2018 20:00 hs', fechaHasta: '02/11/2018 23:00 hs'},
                {nombre: 'Ezequiel' , apellido: 'Braicovich',documento: '39.645.569 ' , fechaDesde: '02/11/2018 20:00 hs', fechaHasta: '02/11/2018 23:00 hs'},
            ]
        }
    }
    
    deleteRow(secId, rowId, rowMap) {
        rowMap[`${secId}${rowId}`].closeRow();
        const newData = [...this.state.listViewData];
        newData.splice(rowId, 1);
        this.setState({ invitadosAutenticados: newData });
      }

    render() {
        const swipeOutSettings = {
            autoClose: true,
            style={backgroundColor:'#fff'},
            right: [{
                text: 'Eliminar', backgroundColor: '#E53520', 
                onPress: () => {
                    Alert.alert(
                        'Alert',
                        'Está seguro que desea eliminar ? ',
                        [
                            {text: 'Cancelar', onPress: () => console.log('Cancel pressed'), style: 'cancel'},
                            {text: 'Aceptar', onPress: () => {
                                flatListData.splice(this.props.index, 1);
                            }},

                        ],
                        {cancelable: true}
                    );
                }
            }],
            rowId : this.props.index,
            sectionId : 1,
            onOpen: (secId, rowId, direction) => {
                this.setState({activeRowKey : this.props.item.key})
            },
            onClose: (secId, rowId, direction) => {
                if (this.state.activeRowKey != null ) {
                    this.setState({activeRowKey : null})
                }
            }
            
        }
        return (
                <Container>
                        <Content scrollEnabled={false}>
                        <Separator bordered>
                            <Text style={{fontSize:12}}> Autenticados</Text>
                        </Separator>
                        <FlatList
                            data={this.state.invitadosAutenticados}
                            renderItem= {({item,index}) => {
                                return (
                                    <FlatListItem item= {item} index= {index}>

                                    </FlatListItem>
                                )
                            }}
                        
                        
                        
                        >
                        </FlatList>
                        <List>
                        {this.state.invitadosAutenticados.map((invitado) => {
                            return (
                                <Swipeout {...swipeOutSettings} >
                                    <ListItem avatar>
                                            <Left>
                                                <Thumbnail source={{ uri: 'data:image/jpeg;base64,' }} />
                                            </Left>
                                            <Body>
                                                <Text> {invitado.nombre + ' ' + invitado.apellido} </Text>
                                                <Text> {invitado.documento} </Text>
                                            </Body>
                                            <Right>
                                                <Text note> {invitado.fechaDesde} </Text>
                                                <Text note> {invitado.fechaHasta} </Text>
                                            </Right>
                                    </ListItem>
                                </Swipeout>
                            );
                            })}
                        </List>
                        <Separator bordered>
                            <Text style={{fontSize:12}}> Sin Autenticar</Text>
                        </Separator>
                        <List>
                        {this.state.invitados.map((invitado) => {
                            return (
                                <Swipeout style={{backgroundColor:'#fff'}}  right={swipeoutBtns} >
                                    <ListItem avatar>
                                            <Left>
                                                <Thumbnail source={{ uri: 'data:image/jpeg;base64,' }} />
                                            </Left>
                                            <Body>
                                                <Text> {invitado.documento} </Text>
                                                <Text></Text>
                                            </Body>
                                            <Right>
                                                <Text note> {invitado.fechaDesde} </Text>
                                                <Text note> {invitado.fechaHasta} </Text>
                                            </Right>
                                    </ListItem>
                                </Swipeout>

                            );
                            })}
                        </List>  
                        </Content>
                </Container>
        );
    }
}

export default Invitaciones;
