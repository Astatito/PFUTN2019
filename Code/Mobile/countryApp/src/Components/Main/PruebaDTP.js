...
class PruebaDTP extends Component {
    ...
    showDTP = () => { // Muestra el DTP.
        this.setState({isDTPVisible: true})
    }

    handleDTP = (datetime) =>  { // Asigna el valor elegido al estado, y oculta el DTP.
        this.setState({
    ...
    hideDTP = () =>  { // Oculta el DTP.
        this.setState({isDTPVisible: false})
    }

    consoleFecha = () => { // Recupera la fecha 20/07/2019 de Firestore (en formato Timestamp), y muestra el objeto Date (Sat Jul 20 2019 00:00:00 GMT-0300)
        var dbRef = Database.collection('PruebaDate')
        var dbDoc = dbRef.doc('Prueba').get()
            .then(doc => {
                if (doc.exists) {
    ...
    grabar = () => { // Graba la fecha en Firestore   
        ...
        return (
            <View>
                ...
                <Card>
                    ...
                    <CardSection>
                        <Button onPress={this.showDTP}>
                            Mostrar DTP
                        </Button>
                        <Button onPress={this.consoleFecha}>
                            Get Fecha (FS)
                        </Button>
                        <Button onPress={this.grabar}>
                            Grabar (FS)
                        </Button>
                    </CardSection>
                </Card>
